...
let ``cooMap fills missing cells (general form)`` () =
    ...
    Assert.Equal(List.tryFind (fun (i, j, _) -> i = 2UL<rowindex> && j = 2UL<colindex>) actual.list,
...
let ``cooMapi position-dependent values`` () =
    ...
    let f i j v = v |> Option.map (fun v -> v + (int (uint64 i)))
...
let ``cooMapi filters None results`` () =
    ...
    let f _i _j v = v |> Option.bind (fun v -> if v > 2 then Some(v * 10) else None)

    let expected =
        CoordinateList(
            nrows,
            ncols,
            [ (0UL<rowindex>, 1UL<colindex>, 50)
              (1UL<rowindex>, 0UL<colindex>, 30) ]
        )
